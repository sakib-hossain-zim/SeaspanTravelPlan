...
router.post('/event/new', function(req, res, next) {
    ...
    pool.query('INSERT INTO event SET ?',postData, function (error, results, fields) {
...
router.get('/travelplan/view',(req,res) => {
  pool.query(sqlquery_travelPlan,(err,results) => {
    if(err){
      return res.send(err);
      console.log(err)
    } else {
...
router.post('/travelplan/new', function(req, res, next) {
    var postData = req.body;
    pool.query('INSERT INTO travelplan SET ?',postData, function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.post('/addTraveller/new', function(req, res, next) {
    var postData = req.body;
    pool.query('INSERT INTO traveller_event SET ?',postData, function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.get('/addTraveller/view',(req,res) => {
  pool.query(sqlquery_addTraveller,(err,results) => {
    if(err){
      return res.send(err);
      console.log(err)
    } else {
...
router.get('/budget/view',(req,res) => {
  pool.query(sqlquery_budget_viewer,(err,results) => {
    if(err){
      return res.send(err);
      console.log(err)
    } else {
...
router.post('/budget/new', function(req, res, next) {
    var postData = req.body;
    pool.query('INSERT INTO budget SET ?',postData, function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.get('/traveller/view',(req,res) => {
  pool.query(sqlquery_traveller_viewer,(err,results) => {
    if(err){
      return res.send(err);
      console.log(err)
    } else {
...
router.put('/traveller/edit', function(req, res, next) {
    pool.query('UPDATE traveller SET `password`=?,`name`=?,`company`=?,`location`=?,`labor_category`=?,`labor_rate`=? where `VSY_IndexNo`=?',[req.body.password,req.body.name,req.body.company,req.body.location,req.body.labor_category,req.body.labor_rate,req.body.VSY_IndexNo], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.post('/traveller/new', function(req, res, next) {
    var postData = req.body;
    pool.query('INSERT INTO traveller SET ?',postData, function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.post('/onlinetravelrequest/new', function(req, res, next) {
    var postData = req.body;
    pool.query('INSERT INTO onlinetravelrequest SET ?',postData, function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.post('/items/new', function(req, res, next) {
    var postData = req.body;
    pool.query('INSERT INTO items SET ?',postData, function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.get('/items/view',(req,res) => {
  pool.query(items_query,(err,results) => {
    if(err){
      return res.send(err);
      console.log(err)
    } else {
...
router.get('/items/view/:VSY_IndexNo',(req,res) => {
  console.log(req)
  pool.query('SELECT * from items where VSY_IndexNo=?', [req.params.VSY_IndexNo], (err,results) => {
    if(err){
      return res.send(err);
      console.log(err)
    } else {
...
router.get('/items/viewer/:Budget_id',(req,res) => {
  console.log(req)
  pool.query('SELECT * from items where Budget_id=?', [req.params.Budget_id], (err,results) => {
    if(err){
      return res.send(err);
      console.log(err)
    } else {
...
router.get('/items/view/:VSY_IndexNo/:TravelPlan_id/:Event_id',(req,res) => {
  console.log(req)
  pool.query('SELECT * from items where VSY_IndexNo=? and TravelPlan_id=? and Event_id=?', [req.params.VSY_IndexNo,req.params.TravelPlan_id,req.params.Event_id], (err,results) => {
    if(err){
      return res.send(err);
      console.log(err)
    } else {
...
router.put('/items/edit', function(req, res, next) {
    pool.query('UPDATE items SET `requested_amount`=?, `check_posted`=? where `Item_id`=?',[req.body.requested_amount,req.body.check_posted,req.body.Item_id], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.put('/items/edit/expense_claimed_amount', function(req, res, next) {
    pool.query('UPDATE items SET `expense_claimed_amount`=? where `Item_id`=?',[req.body.expense_claimed_amount,req.body.Item_id], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.put('/items/edit_approved_amountandnotes', function(req, res, next) {
    pool.query('UPDATE items SET `approved_amount`=?, `note_from_coordinator`=? where `Item_id`=?',[req.body.approved_amount,req.body.note_from_coordinator,req.body.Item_id], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.put('/otr/otr_edit', function(req, res, next) {
    pool.query('UPDATE onlinetravelrequest SET `status`=? where `Request_Form_No`=?',[req.body.status,req.body.Request_Form_No], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.get('/itemsotr/view',(req,res) => {
  pool.query(items_onlinetravelrequest_query,(err,results) => {
    if(err){
      return res.send(err);
      console.log(err)
    } else {
...
router.get('/budget_selection/view',(req,res) => {
  pool.query('SELECT traveller_event.Event_id, traveller_event.VSY_IndexNo, traveller_event.travel_start_date, traveller_event.travel_end_date,  event.event_name, event.event_start_date, event.event_end_date, event.TravelPlan_id, travelplan.nss_program, traveller.name from (((traveller_event Inner Join event on traveller_event.Event_id = event.Event_id) inner join traveller on traveller_event.VSY_IndexNo = traveller.VSY_IndexNo) inner join travelplan on event.TravelPlan_id = travelplan.TravelPlan_id)',(err,results) => {
    if(err){
      return res.send(err);
      console.log(err)
    } else {
...
router.post('/travel_auth/new', function(req, res, next) {
    var postData = req.body;
    pool.query('INSERT INTO authorizationplan SET ?',postData, function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.post('/login/auth/',function(req,res){
  ...
  pool.query('SELECT * from traveller WHERE `VSY_IndexNo` =?', [VSY_IndexNo,password], function(error,results,fields){
    if(error) {
      res.send({
        "code": 400,
  ...
  router.post('/login/coordinator/auth/',function(req,res){
    var VSY_IndexNo = req.body.VSY_IndexNo;
    var password = req.body.password;
    pool.query('SELECT * from coordinator WHERE `VSY_IndexNo` =?', [VSY_IndexNo,password], function(error,results,fields){
      if(error) {
        res.send({
          "code": 400,
          "failed": "error occurred"
  ...
  router.get('/authorizationplan/view/:VSY_IndexNo',(req,res) => {
    console.log(req)
    pool.query('SELECT * from authorizationplan where VSY_IndexNo=?', [req.params.VSY_IndexNo], (err,results) => {
      if(err){
        return res.send(err);
        console.log(err)
      } else {
  ...
  router.get('/otr/view',(req,res) => {
    pool.query(otr_query,(err,results) => {
      if(err){
        return res.send(err);
        console.log(err)
      } else {
...
router.post('/travelplan/delete', function(req, res, next) {
    pool.query('DELETE t,e,b,i from travelplan as t INNER JOIN event as e on t.TravelPlan_id = e.TravelPlan_id INNER JOIN budget as b on t.TravelPlan_id = b.TravelPlan_id INNER JOIN items as i on t.TravelPlan_id = i.TravelPlan_id where t.TravelPlan_id=?',[req.body.TravelPlan_id], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.post('/travelplan/delete/individual', function(req, res, next) {
    pool.query('DELETE from travelplan where `TravelPlan_id`=?',[req.body.TravelPlan_id], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.post('/event/delete', function(req, res, next) {
    pool.query('DELETE e,b,i from event as e INNER JOIN budget as b on e.Event_id = b.Event_id INNER JOIN items as i on e.Event_id = i.Event_id where e.Event_id=?',[req.body.Event_id], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.post('/event/delete/individual', function(req, res, next) {
    pool.query('DELETE from event where `Event_id`=?',[req.body.Event_id], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.post('/budget/delete', function(req, res, next) {
    pool.query('DELETE b,i from budget as b INNER JOIN items as i on b.Budget_id = i.Budget_id  where b.Budget_id=?',[req.body.Budget_id], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.post('/budget/delete/individual', function(req, res, next) {
    pool.query('DELETE from budget where `Budget_id`=?',[req.body.Budget_id], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.post('/items/delete', function(req, res, next) {
    pool.query('DELETE from items where `Item_id`=?',[req.body.Item_id], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
router.put('/items_all/edit', function(req, res, next) {
    pool.query('UPDATE items SET `Budget_id`=?, `TravelPlan_id`=?, `VSY_IndexNo`=?, `Event_id`=?, `item_name`=?, `amount`=?, `comment`=? where `Item_id`=?',[req.body.Budget_id,req.body.TravelPlan_id,req.body.VSY_IndexNo,req.body.Event_id,req.body.item_name,req.body.amount,req.body.comment,req.body.Item_id], function (error, results, fields) {
        if(error) throw error;
        res.send(JSON.stringify(results));
    });
});
...
const multerConfig = multer.diskStorage({
  ...
  router.post('/upload',multer(multerConfig).single('photo'),function(req,res){
    ...
    router.post('/travelexpenseclaim/new', function(req, res, next) {
        var postData = req.body;
        pool.query('INSERT INTO travelexpenseclaim SET ?',postData, function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.put('otr/edit/changeotrstatus', function(req, res, next) {
        pool.query('UPDATE onlinetravelrequest SET `submit_status`=? where `Request_Form_No`=?',[req.body.submit_status,req.body.Request_Form_No], function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.get('/otr/check/view/:VSY_IndexNo/:TravelPlan_id/:Event_id',(req,res) => {
      pool.query('SELECT * from onlinetravelrequest where `VSY_IndexNo`=? and `TravelPlan_id`=? and `Event_id`=?',[req.params.VSY_IndexNo,req.params.TravelPlan_id,req.params.Event_id],(err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        } else {
    ...
    router.put('/auth_travelexpensestatus/change/edit', function(req, res, next) {
        pool.query('UPDATE authorizationplan SET `status3`=? where `Travel_Auth_No`=?',[req.body.status3,req.body.Travel_Auth_No], function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.put('/traveller_event/change/edit', function(req, res, next) {
        pool.query('UPDATE traveller_event SET `otr_request_status`=? where `Event_id`=? and `VSY_IndexNo`=?',[req.body.otr_request_status,req.body.Event_id,req.body.VSY_IndexNo], function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.get('/traveller_event/view/:VSY_IndexNo/:Event_id',(req,res) => {
      pool.query('SELECT * from traveller_event where `VSY_IndexNo`=? and `Event_id`=?',[req.params.VSY_IndexNo,req.params.Event_id],(err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        } else {
    ...
    router.get('/traveller_event_event/view/:VSY_IndexNo/:Event_id',(req,res) => {
      pool.query('SELECT traveller_event.Event_id, traveller_event.VSY_IndexNo, event.event_start_date, event.event_end_date from (traveller_event inner join event on event.Event_id = traveller_event.Event_id) where `VSY_IndexNo`=? and traveller_event.Event_id=?',[req.params.VSY_IndexNo,req.params.Event_id],(err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        } else {
    ...
    router.get('/travelprogram/view', (req,res) => {
      pool.query('SELECT * from travelprograms', (err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        }else {
    ...
    router.get('/budgetedit_selection/view/:nss_program',(req,res) => {
      pool.query('SELECT event.Event_id, event.event_name, event.event_start_date, event.event_end_date, event.TravelPlan_id, travelplan.nss_program from (event inner join travelplan on event.TravelPlan_id = travelplan.TravelPlan_id) where `nss_program`=?',[req.params.nss_program],(err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        } else {
    ...
    router.get('/budgetedit_traveller_selection/view/:Event_id',(req,res) => {
      pool.query('SELECT traveller_event.VSY_IndexNo,traveller_event.Event_id,traveller.name,traveller_event.travel_start_date,traveller_event.travel_end_date, event.event_name from  ((traveller_event inner join event on event.Event_id = traveller_event.Event_id) inner join traveller on traveller.VSY_IndexNo = traveller_event.VSY_IndexNo)  where traveller_event.Event_id=?',[req.params.Event_id],(err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        } else {
    ...
    router.get('/items/viewer_for_budgetedit/:VSY_IndexNo/:Event_id/:TravelPlan_id',(req,res) => {
      console.log(req)
      pool.query('SELECT * from items where `VSY_IndexNo`=? and `Event_id`=? and `TravelPlan_id`=?',[req.params.VSY_IndexNo,req.params.Event_id,req.params.TravelPlan_id],(err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        } else {
    ...
    router.put('/edit_travel_status', function(req, res, next) {
        pool.query('UPDATE authorizationplan SET `status2_bool`=? where `Travel_Auth_No`=?',[req.body.status2_bool,req.body.Travel_Auth_No], function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.put('/edit_travel_end_date', function(req, res, next) {
        pool.query('UPDATE traveller_event SET `travel_end_date`=? where `VSY_IndexNo`=? and `Event_id`=?',[req.body.travel_end_date,req.body.VSY_IndexNo, req.body.Event_id], function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.get('/sub_items_data/:Item_id',(req,res) => {
      console.log(req)
      pool.query('SELECT * from subitems where `Item_id`=? and `coordinator_approval_status`="REJECTED"',[req.params.Item_id,req.params.coordinator_approval_status],(err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        } else {
    ...
    router.post('/submit_subItem/new', function(req, res, next) {
        var postData = req.body;
        pool.query('INSERT INTO subitems SET ?',postData, function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.post('/uploadToSql', function(req, res, next) {
        var postData = req.body;
        pool.query('INSERT INTO data_test SET ?',postData, function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.get('/data_test_view', (req,res) => {
      pool.query('SELECT * from data_test', (err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        }else {
    ...
    router.put('/traveller_event/expense_claim/change/edit', function(req, res, next) {
        pool.query('UPDATE traveller_event SET `travel_expense_claim_status`=? where `Event_id`=? and `VSY_IndexNo`=?',[req.body.travel_expense_claim_status,req.body.Event_id,req.body.VSY_IndexNo], function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.get('/travellertype/view',(req,res) => {
      console.log(req)
      pool.query('SELECT * from traveller_type',(err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        } else {
    ...
    router.put('/subitem/edit_coordinator_approval_status/', function(req, res, next) {
        pool.query('UPDATE subitems SET `coordinator_approval_status`=? , `rejection_reasoning`=?  where `Sub_Item_id`=?',[req.body.coordinator_approval_status,req.body.rejection_reasoning,req.body.Sub_Item_id], function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.put('/item/expenseclaim_approval_status/', function(req, res, next) {
        pool.query('UPDATE items SET `expenseclaim_approval`=? where `Item_id`=?',[req.body.expenseclaim_approval,req.body.Item_id], function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.get('/subitems/data', function(req,res,next) {
      pool.query('Select * from subitems', function(err, results, fields){
        if(err) {
          return res.send(err);
          console.log(err)
        } else {
    ...
    router.put('/travelexpenseclaim/status_update/', function(req, res, next) {
        pool.query('UPDATE travelexpenseclaim SET `coordinator_approval`=? where `Invoice_No`=?',[req.body.coordinator_approval,req.body.Invoice_No], function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.get('/subitems/pending_data', function(req,res,next) {
      pool.query('Select * from subitems where `coordinator_approval_status`="PENDING"', function(err, results, fields){
        if(err) {
          return res.send(err);
          console.log(err)
        } else {
    ...
    router.get('/travelexpenseclaim/items/view/:VSY_IndexNo/:TravelPlan_id/:Event_id',(req,res) => {
      console.log(req)
      pool.query('SELECT * from items where VSY_IndexNo=? and TravelPlan_id=? and Event_id=? and `expenseclaim_approval`="REJECTED"', [req.params.VSY_IndexNo,req.params.TravelPlan_id,req.params.Event_id,req.params.expenseclaim_approval], (err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        } else {
    ...
    router.get('/subitems/data/:Item_id',(req,res) => {
      console.log(req)
      pool.query('SELECT * from subitems where `Item_id`=?',[req.params.Item_id,req.params.coordinator_approval_status],(err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        } else {
    ...
    router.put('/travelexpenseclaim/items/status/allupdate/', function(req, res, next) {
        pool.query('UPDATE items SET `expenseclaim_approval`=?     where `VSY_IndexNo`= ? and `Event_id`= ? and `TravelPlan_id`=? ',[req.body.expenseclaim_approval, req.body.VSY_IndexNo, req.body.Event_id,req.body.TravelPlan_id], function (error, results, fields) {
            if(error) throw error;
            res.send(JSON.stringify(results));
        });
    });
    ...
    router.get('/subitems/data/view/:VSY_IndexNo/:Event_id/:TravelPlan_id',(req,res) => {
      pool.query('SELECT * from subitems where `VSY_IndexNo`=? and `Event_id`= ? and `TravelPlan_id`= ?',[req.params.VSY_IndexNo,req.params.Event_id,req.params.TravelPlan_id],(err,results) => {
        if(err){
          return res.send(err);
          console.log(err)
        } else {
